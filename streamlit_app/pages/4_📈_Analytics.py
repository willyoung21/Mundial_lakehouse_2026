...
from utils.charts import xg_scatter, champion_odds_bars, results_donut
...
with tab_xg:
    ...
    if df_xg.empty:
        st.info("No hay datos de xG para WC2026 todavía. Los datos de xG provienen de StatsBomb (disponibles al finalizar el torneo).")
        ...
        with st.expander("Ver tabla de datos"):
            cols_show = ["team_canonical", "confederation", "avg_xg_per_match", "ga_per_match", "matches_played"]
            st.dataframe(
                df_xg[[c for c in cols_show if c in df_xg.columns]].rename(columns={
                    "team_canonical": "Equipo", "confederation": "Conf",
                    "avg_xg_per_match": "xG/partido", "ga_per_match": "GA/partido",
                    "matches_played": "PJ",
                }),
                hide_index=True,
    ...
    if not df_comp.empty:
        ...
        fig_comp = px.bar(
            df_comp, x="competition_slug", y="avg_xg",
            labels={"competition_slug": "Competición", "avg_xg": "xG promedio / partido"},
            color="avg_xg", color_continuous_scale="Blues",
...
with tab_odds:
    ...
    if df_odds.empty:
        st.info("No hay datos de Onside Arena disponibles. Ejecuta: `python -m ingestion.onside_loader`")
    else:
        ...
        with st.expander("Ver tabla completa de probabilidades"):
            cols_show = ["champion_rank", "team_canonical", "confederation",
                         "champion_pct", "reach_final_pct", "reach_semi_pct", "reach_qf_pct"]
            display = df_odds[[c for c in cols_show if c in df_odds.columns]].rename(columns={
                "champion_rank": "#", "team_canonical": "Equipo", "confederation": "Conf",
                "champion_pct": "Campeón %", "reach_final_pct": "Final %",
                "reach_semi_pct": "Semi %", "reach_qf_pct": "Cuartos %",
            })
            st.dataframe(
                display,
                hide_index=True,
...
with tab_dist:
    ...
    def load_results() -> tuple[dict, dict]:
        ...
        hist = {row["result"]: round(row["n"] / hist_total * 100, 1) for _, row in df_hist.iterrows()}
        return wc, hist
    ...
    if not wc_counts:
        ...
        col1.metric("Victoria local", f"{wc_counts.get('home_win', 0)}", f"{wc_counts.get('home_win', 0)/total*100:.1f}%")
        col2.metric("Empates", f"{wc_counts.get('draw', 0)}", f"{wc_counts.get('draw', 0)/total*100:.1f}%")
        col3.metric("Victoria visitante", f"{wc_counts.get('away_win', 0)}", f"{wc_counts.get('away_win', 0)/total*100:.1f}%")
        ...
        if not df_by_stage.empty:
            ...
            df_by_stage["% visit."] = (df_by_stage["visitante"] / df_by_stage["total"] * 100).round(1)
            st.dataframe(
                df_by_stage[["stage", "total", "local", "empates", "visitante", "% local", "% empate", "% visit."]].rename(
                    columns={"stage": "Fase", "total": "Total", "local": "Local", "empates": "Empate", "visitante": "Visitante"}
                ),
                hide_index=True,
                use_container_width=True,
            )
