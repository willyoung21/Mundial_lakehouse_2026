        ...
        records.append(
            {
                ...
                "ga": as_,
                "pts": hp,
                "wins": 1 if hp == 3 else 0,
                "draws": 1 if hp == 1 else 0,
                "losses": 1 if hp == 0 else 0,
                "pj": 1,
            }
        )
        records.append(
            {
                ...
                "ga": hs,
                "pts": ap,
                "wins": 1 if ap == 3 else 0,
                "draws": 1 if ap == 1 else 0,
                "losses": 1 if ap == 0 else 0,
                "pj": 1,
            }
        )
...
def _predict(home: str, away: str) -> dict | None:
    if not home or not away or "?" in (home, away):
        return None
    try:
        r = requests.post(f"{API_URL}/predict/winner", json={"home": home, "away": away}, timeout=8)
        ...
        with cols_grid[gi % 3]:
            ...
            for _, match in gdf.iterrows():
                ...
                if pd.notna(match["home_score"]):
                    ...
                    new_h = c2.number_input(
                        "L", 0, 20, int(prev[0]), key=f"h_{mid}", label_visibility="collapsed"
                    )
                    new_a = c3.number_input(
                        "V", 0, 20, int(prev[1]), key=f"a_{mid}", label_visibility="collapsed"
                    )
            ...
            for j, row in standings.iterrows():
                ...
                st.markdown(
                    f"{icon} **{row['team']}** &nbsp; {pts}pts &nbsp; {gf}–{ga} (DG {dg:+d})"
                )
            st.markdown("")
...
with tab_bracket:
    ...
    if thirds_data2:
        best_thirds2 = (
            pd.DataFrame(thirds_data2)
            .sort_values(["pts", "dg", "gf"], ascending=False)["team"]
            .tolist()[:8]
        )
    else:
        best_thirds2 = []
    ...
    R32_PAIRS = [
        ...
        (_q(G["K"], 0), _q(G["L"], 1)),
        (_q(G["L"], 0), _q(G["K"], 1)),
        (best_thirds2[0], best_thirds2[1]),
        (best_thirds2[2], best_thirds2[3]),
        (best_thirds2[4], best_thirds2[5]),
        (best_thirds2[6], best_thirds2[7]),
    ]
    ...
    def _match_card(round_key: str, home: str, away: str) -> str:
        ...
        if pred:
            fig = probability_bars(
                home, away, pred["home_win_pct"], pred["draw_pct"], pred["away_win_pct"], height=85
            )
    ...
    if len(sf_winners) >= 2:
        ...
        with col_champ:
            ...
            if pred_fin:
                pct = (
                    pred_fin["home_win_pct"]
                    if champion == sf_winners[0]
                    else pred_fin["away_win_pct"]
                )
