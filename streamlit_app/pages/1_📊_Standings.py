...
for grp in groups:
    ...
    if len(gdf) >= 3:
        thirds.append({"team": gdf.iloc[2]["team"], "pts": gdf.iloc[2]["pts"],
                        "dg": gdf.iloc[2]["dg"], "gf": gdf.iloc[2]["gf"]})
if thirds:
    thirds_sorted = sorted(thirds, key=lambda x: (-x["pts"], -x["dg"], -x["gf"]))
    third_threshold = thirds_sorted[min(7, len(thirds_sorted)-1)]["pts"] if len(thirds_sorted) >= 8 else 0
else:
    third_threshold = 0
